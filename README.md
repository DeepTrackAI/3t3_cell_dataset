...
The file format, `.pkl`, is powered by the `joblib` package in Python, and `NumPy` for `.npy` format. Each curvers in the two `.pkl` files are paired, as they are collected in one test including both the approach and reconstruction process. Also, the order of the labels should be consistent with the curve.
